...
function EditarEncuestaContent({ surveyId }: { surveyId: string }) {
    ...
    const cargarEncuesta = async () => {
        ...
        setDescripcion(survey.descripcion || "")
        ...
        if (questions) {
            setPreguntas(
                questions.map((q) => ({
                    id: generarId(),
                    db_id: q.id,
                    tipo: q.tipo as "abierta" | "cerrada",
                    pregunta: q.pregunta,
                    opciones: q.opciones || [],
                    requerida: q.requerida || false,
                }))
            )
        }
    ...
    const actualizarPregunta = (id: string, campo: Partial<Pregunta>) => {
        setPreguntas(preguntas.map((p) => (p.id === id ? { ...p, ...campo } : p)))
    }
    ...
    const actualizarOpcion = (preguntaId: string, index: number, value: string) => {
        setPreguntas(
            preguntas.map((p) =>
                p.id === preguntaId
                    ? { ...p, opciones: p.opciones.map((o, i) => (i === index ? value : o)) }
                    : p
            )
        )
    }
    ...
    const guardarEncuesta = async () => {
        ...
        if (surveyError) {
            setError("Error actualizando la encuesta")
            setGuardando(false)
            return
        }
        ...
        if (questionsError) {
            setError("Error guardando las preguntas")
            setGuardando(false)
            return
        }
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-neutral-500">
                            Modifica tu encuesta y sus preguntas
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white border border-neutral-200 rounded-2xl p-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm text-neutral-500 mb-1.5">Descripción (opcional)</label>
                                <textarea
                                    value={descripcion}
                                    ...
                                    <div className="flex items-start gap-3 mb-3">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <label className="flex items-center gap-1.5 ml-auto text-xs text-neutral-500 cursor-pointer">
                                                    <input
                                                        ...
                                                        onChange={(e) =>
                                                            actualizarPregunta(pregunta.id, { requerida: e.target.checked })
                                                        }
                                            ...
                                            <input
                                                ...
                                                onChange={(e) =>
                                                    actualizarPregunta(pregunta.id, { pregunta: e.target.value })
                                                }
                                                    ...
                                                    {pregunta.opciones.map((opcion, opIndex) => (
                                                        <div key={opIndex} className="flex items-center gap-2">
                                                            ...
                                                            <input
                                                                ...
                                                                value={opcion}
                                                                onChange={(e) =>
                                                                    actualizarOpcion(pregunta.id, opIndex, e.target.value)
                                                                }
                                                            ...
                                                            {pregunta.opciones.length > 2 && (
                                                                <button
                                                                    onClick={() => eliminarOpcion(pregunta.id, opIndex)}
    ...
    return (
        <AuthGuard>
            <Suspense fallback={null}>
                <EditarEncuestaContent surveyId={resolvedParams.id} />
            </Suspense>
        </AuthGuard>
    )
}
