...
const WA_LINK = "https://wa.me/523336821088?text=Hola%2C%20me%20gustar%C3%ADa%20conocer%20m%C3%A1s%20sobre%20El%20Romeral"
    ...
    <>
      ...
      {open && !chatOpen && (
        <>
          ...
          />
          <div
            className="fixed bottom-24 right-6 z-[69] flex flex-col gap-2"
            style={{ animation: "waPopup .22s cubic-bezier(.4,0,.2,1) forwards" }}
          >
            {/* Opción 1: Chat aquí */}
            <button
              onClick={() => { setOpen(false); setChatOpen(true) }}
              className="flex items-center gap-3 bg-background/95 backdrop-blur-sm border border-primary/15 shadow-xl px-4 py-3 text-left hover:bg-primary/5 transition-colors duration-200 group"
              style={{ borderRadius: 12, minWidth: 220 }}
            >
              <span className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary flex-shrink-0 text-sm">💬</span>
              <div>
                <div className="text-[11px] font-medium tracking-wide text-foreground">Asesor aquí mismo</div>
                <div className="text-[10px] text-muted-foreground">Chat en el sitio</div>
              </div>
            </button>

            {/* Opción 2: Abrir WhatsApp */}
            <a
              href={WA_LINK}
              target="_blank"
              rel="noopener noreferrer"
              onClick={() => setOpen(false)}
              className="flex items-center gap-3 bg-background/95 backdrop-blur-sm border border-primary/15 shadow-xl px-4 py-3 hover:bg-[#25D366]/5 transition-colors duration-200 group"
              style={{ borderRadius: 12, minWidth: 220 }}
            >
              <span className="w-8 h-8 rounded-full bg-[#25D366]/15 flex items-center justify-center flex-shrink-0">
              ...
              <div>
                <div className="text-[11px] font-medium tracking-wide text-foreground">Abrir WhatsApp</div>
                <div className="text-[10px] text-muted-foreground">Escríbenos directo</div>
              </div>
            </a>
          </div>
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        aria-label="Contactar a El Romeral"
        className="fixed bottom-8 right-6 z-[70] group flex items-center gap-3"
      >
        {/* Tooltip — solo cuando el popup está cerrado */}
        {!open && (
          <span className="opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 transition-all duration-300 text-[10px] tracking-[0.18em] uppercase font-light bg-background/90 backdrop-blur-sm border border-primary/15 text-foreground px-3 py-1.5 shadow-lg whitespace-nowrap">
            Escríbenos
          </span>
        )}
        {/* Botón */}
        <div className="relative flex items-center justify-center" style={{ width: 52, height: 52 }}>
          <span className="absolute inline-flex h-full w-full rounded-full bg-[#25D366]/30 animate-ping opacity-60" />
          <div
            className="relative rounded-full bg-[#25D366] shadow-lg shadow-[#25D366]/30 transition-all duration-300 flex items-center justify-center border border-[#25D366]/20"
            style={{
              width: 52, height: 52,
              transform: open ? "rotate(45deg) scale(1.05)" : undefined,
            }}
          >
            {open ? (
